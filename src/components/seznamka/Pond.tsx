...
import { Heart, X, Skull, User, Users, Flag, MessageCircle, Bookmark, ShieldCheck, Crosshair, Crown, Fish, Layers, DollarSign, CircleDollarSign, Dumbbell, Wine, Filter, Search, Sparkles, Calendar, TrendingUp, MapPin, ChevronDown, Repeat, Check, Flame, Zap, Diamond } from "lucide-react";
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (paymentStatus === 'success' && sessionId) {
        ...
        setPaymentError({ title: lang === 'cs' ? 'Zrušeno' : 'Cancelled', message: lang === 'cs' ? 'Platba byla zrušena.' : 'Payment was cancelled.' });
        window.history.replaceState({}, document.title, window.location.pathname);
      }
    }
  }, [lang]);
                  ...
                  {loading ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 border border-mafia-gold/20 rounded-3xl p-6 text-center z-0 backdrop-blur-md">
                      ...
                      <div className="flex gap-4 w-full max-w-md">
                        ...
                        >
                          <MessageCircle size={24} className="text-white/70 group-hover:scale-110 transition-transform" />
          ...
          <AnimatePresence>
            {paymentError && (
              <motion.div
                ...
                className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm"
              >
                <div className="bg-[#111] border border-red-500/30 rounded-2xl p-8 max-w-sm w-full text-center shadow-[0_0_50px_rgba(239,68,68,0.2)]">
                  <div className="w-20 h-20 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-6 border border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.4)]">
                    <X size={40} className="text-red-500" />
                  </div>
                  <h2 className="text-2xl font-black font-heading text-red-500 mb-2 uppercase tracking-wider">
                    {paymentError.title}
                  </h2>
                  <p className="text-gray-300 mb-8 font-mono">
                  ...
                  <button
                    ...
                    className="w-full bg-red-500 text-white font-black font-heading tracking-widest py-3 rounded-lg hover:bg-red-600 transition-all hover:scale-105 shadow-[0_0_20px_rgba(239,68,68,0.4)] uppercase"
                  >
                ...
                >
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-2xl font-heading font-black text-white uppercase tracking-widest flex items-center gap-3">
                      <MessageCircle size={28} className="text-mafia-gold" />
