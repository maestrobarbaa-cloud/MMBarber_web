...
// 6. Add isDsaModalOpen state
content = content.replace(
  /const \[isPlayingAudio, setIsPlayingAudio\] = useState\(false\);/,
  `const [isPlayingAudio, setIsPlayingAudio] = useState(false);\n  const [isDsaModalOpen, setIsDsaModalOpen] = useState(false);`
);

// 7. Add Report Button to Header
const oldHeader = `<div className="bg-mafia-dark/90 p-4 flex justify-between items-center border-b border-white/10 z-40 shrink-0">
...
const newHeader = `<div className="bg-mafia-dark/90 p-4 flex justify-between items-center border-b border-white/10 z-40 shrink-0">
        <h3 className="font-heading font-black uppercase text-mafia-gold tracking-widest">
          Profil: {profile.name}
        </h3>
        <div className="flex items-center gap-2">
          <button 
            ...
            className="text-red-500/70 hover:text-red-500 hover:bg-red-500/10 p-2 rounded-full transition-colors flex items-center justify-center"
            title={lang === 'cs' ? 'Nahlásit profil (DSA)' : 'Report Profile (DSA)'}
          >
...
// 8. Add DsaReportModal before </AnimatePresence> inside ProfileCard return
content = content.replace(
  ...
  `</AnimatePresence>
      ...
      <DsaReportModal 
        ...
        reportedProfileId={profile.id || 'unknown'}
...
console.log('Done modifying ProfileCard.tsx completely');
