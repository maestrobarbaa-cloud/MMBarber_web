...
content = content.replace(
  /export function TomasSkillTree\(\{\s*totalCollected,\s*lang\s*\}\:\s*\{\s*totalCollected:\s*number;\s*lang:\s*string;\s*\}\)\s*\{/,
...
// Background and Grid
content = content.replace(
  /backgroundImage:\s*"radial-gradient\(circle at center, rgba\(197,160,89,0\.25\) 0%, transparent 70%\)"/,
...
content = content.replace(
  /border-mafia-gold text-mafia-gold/g,
  `\${bc} \${tc}`
);
content = content.replace(
  /bg-mafia-gold\/20/g,
...
content = content.replace(
  /border-mafia-gold\/60/g,
  `\${bc}/60`
);
content = content.replace(
  /hover:text-mafia-gold/g,
  `hover:\${tc}`
);
content = content.replace(
  /hover:border-mafia-gold\/30/g,
  `hover:\${bc}/30`
);
content = content.replace(
  /bg-mafia-gold\/10 border-mafia-gold text-mafia-gold shadow-\[0_0_30px_rgba\(197,160,89,0\.5\)\]/g,
  `\${bgc}/10 \${bc} \${tc} shadow-[0_0_30px_rgba(\${primaryColorHex},0.5)]`
);
content = content.replace(
  /drop-shadow-\[0_0_15px_rgba\(197,160,89,0\.9\)\]/g,
  `drop-shadow-[0_0_15px_rgba(\${primaryColorHex},0.9)]`
);
content = content.replace(
  /text-mafia-gold text-shadow-md/g,
  `\${tc} drop-shadow-md`
);
content = content.replace(
  /bg-gradient-to-r from-mafia-gold\/60 via-mafia-gold\/20/g,
  `bg-gradient-to-r from-\${bgc.replace('bg-','')}/60 via-\${bgc.replace('bg-','')}/20`
);
content = content.replace(
  /text-mafia-gold\/60/g,
  `\${tc}/60`
);
content = content.replace(
  /border-mafia-gold\/80 bg-mafia-gold\/5/g,
  `\${bc}/80 \${bgc}/5`
);
content = content.replace(
  /bg-mafia-gold\/15/g,
...
content = content.replace(
  /border-mafia-gold\/40/g,
  `\${bc}/40`
);
content = content.replace(
  /rgba\(197,160,89,0\.2\)/g,
